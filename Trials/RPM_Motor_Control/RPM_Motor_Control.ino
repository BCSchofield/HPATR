...
const long CLEAN_TRAVEL_STEPS = CLEAN_TRAVEL_MM * steps_per_mm;
...
void loop() {
  ...
  if (stringComplete) {
    ...
    if (inputString.indexOf("PRESSURE_OFF") != -1) {
    ...
    // Check for CLEAN command - move to absolute 78mm (bypass 73mm limit)
    else if (inputString.indexOf("CLEAN:") != -1) {
      debugLog("CLEAN command received - moving to 78mm absolute position");
      long currentPos = stepper.currentPosition();
      long targetPos = CLEAN_TRAVEL_STEPS;
      
      // Ensure stepper is properly configured
      stepper.enableOutputs();
      stepper.setMaxSpeed(10000); // safe cleaning speed
      stepper.setAcceleration(1000 * steps_per_mm);
      
      // Set up position tracking for cleaning
      ...
      Serial.println("MOVEMENT_START: Pos=" + String(initialPosition) + " Target=" + String(targetPos) + " Steps=" + String(targetPos - currentPos));
      
      // Move to absolute target
      stepper.moveTo(targetPos);
      
      // Timeout based on distance and cleaning speed
      long stepsToMove = labs(targetPos - currentPos);
      movementStartTime = millis();
      movementTimeout = (long)((stepsToMove / (float)10000) * 1000.0f) + 30000L; // based on cleaning speed
      debugLog("Cleaning movement timeout set to: " + String(movementTimeout) + "ms");
    }
...
void executeMovement(int speed, int distance) {
  ...
  // Validate that we don't exceed the 73mm limit
  if (targetPos < 0 || targetPos > MAX_TRAVEL_STEPS) {
    Serial.println("ERROR: Movement would exceed limits (0-73mm from home)");
    debugLog("Current position: " + String(currentPos) + ", Target would be: " + String(targetPos) + ", Limit steps: " + String(MAX_TRAVEL_STEPS));
    return;
  }
