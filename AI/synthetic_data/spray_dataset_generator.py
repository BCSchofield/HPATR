...
def draw_circular_droplet_realistic(
    ...
    # REALISM: Apply size-dependent edge blur for soft edges
    blur_sigma = radius * rng.uniform(0.05, 0.15)  # Size-dependent blur
    blur_sigma = np.clip(blur_sigma, DROPLET_EDGE_BLUR_SIGMA_MIN, DROPLET_EDGE_BLUR_SIGMA_MAX)
    ksize = int(6 * blur_sigma + 1)
    if ksize % 2 == 0:
        ksize += 1
    
    # Blur only the droplet region
...
def draw_elliptical_droplet_realistic(
    ...
    # REALISM: Apply edge blur
    blur_sigma = avg_radius * rng.uniform(0.05, 0.15)
    blur_sigma = np.clip(blur_sigma, DROPLET_EDGE_BLUR_SIGMA_MIN, DROPLET_EDGE_BLUR_SIGMA_MAX)
    ksize = int(6 * blur_sigma + 1)
    if ksize % 2 == 0:
        ksize += 1
...
def generate_synthetic_image(
    ...
    for _ in range(num_droplets):
        ...
        # REALISM: Draw realistic droplet with absorption profile
        if params['type'] == 'circular':
            ...
            image = draw_elliptical_droplet_realistic(
                ...
                params['angle'], float(bg_intensity), rng
            )
...
def generate_dataset(
    ...
    for i in range(num_images):
        image_id = i + 1
        image_filename = f"image_{image_id:04d}.png"
        ...
        vis_path = visualizations_dir / f"image_{image_id:04d}_vis.png"
